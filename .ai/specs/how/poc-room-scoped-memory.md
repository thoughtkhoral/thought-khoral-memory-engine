...
- Parser supersession: root [008 — Slash decisions and facilitator boundary](https://github.com/thoughtkhoral/thought-khoral/blob/main/.ai/specs/decisions/008-slash-decisions-and-facilitator-boundary.md)
...
The deterministic `Decision:` parser is retired; no automatic prefix proposal
implementation is active. `thought-khoral-memory-engine` is the intended
later implementation of the same port. It derives room-scoped graph facts
and, when an approved plan enables it, returns draft proposals to the gateway
through that port. It must not open a second independent derived-draft path.
Human creation through `/decisions` and `decision.propose` is a distinct,
...
3. The retired `Decision:` parser does not produce a draft. Until a later
   implementation is approved, the port produces no automatic prefix draft.
...
- Extraction failure must not block chat, join, human `/decisions` actions,
  or human transitions.
...
- Regression proof that `Decision:` chat text does not produce a draft and
  that later derived drafts can enter only through the gateway-owned port.
